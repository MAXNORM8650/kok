...
// Global server management for LlamaCpp
class LlamaCppServerManager {
  ...
  async startServer(config: Config): Promise<void> {
    ...
    if (!llamaDir) {
      throw new Error("LLAMA_DIR environment variable not set. Please set it to your llama.cpp directory path.");
    }
    ...
    try {
      ...
      if (response.ok) {
        // console.log(`✅ Using existing llama-server on port ${this.port}`);
        this.isRunning = true;
        return;
      }
    } catch {
    ...
    // console.log(`🚀 Starting llama-server with ${config.model}...`);
    ...
    throw new Error("Failed to start Llama.cpp server after 30 seconds");
  }
...
function getConfig(): Config {
  ...
  if (!fs.existsSync(configPath)) {
    try {
      ...
      console.log("Please edit the config file to add your API keys and preferences.");
...
if (!commandDescription) {
  ...
  console.error("Usage: kok <command description>");
  console.error("\nExamples:");
  console.error("  kok list all files in this directory");
  console.error("  kok create a new directory called 'projects'");
  console.error("  kok find all .js files modified in the last 7 days");
  process.exit(1);
}
...
- Use common Unix/Linux commands that work across different systems
...
`;
  ...
  switch (config.type) {
    ...
    default:
      ...
      console.error("Supported types: OpenAI, Custom, Claude, Gemini, LlamaCpp");
      process.exit(1);
  }
}
...
try {
  ...
  if (error.message.includes("LLAMA_DIR")) {
    console.error("\n💡 To use LlamaCpp, please:");
    console.error("1. Install llama.cpp: https://github.com/ggerganov/llama.cpp");
    console.error("2. Build it with: make llama-server");
    console.error("3. Set LLAMA_DIR: export LLAMA_DIR=/path/to/llama.cpp");
  }
